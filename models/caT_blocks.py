...
class UNetMidBlock3DCrossAttn(nn.Module):
    ...
    def forward(
        ...
        if self.gradient_checkpointing:
            ...
            hidden_states = torch.utils.checkpoint.checkpoint(create_custom_forward(self.temp_convs[0]), hidden_states, num_frames=num_frames)
        else:
        ...
        for attn, cond_attn, temp_attn, resnet, temp_conv in zip(self.attentions, self.cond_attentions, self.temp_attentions, self.resnets[1:], self.temp_convs[1:]):
            if self.gradient_checkpointing:
                ...
                hidden_states = torch.utils.checkpoint.checkpoint(create_custom_forward(cond_attn, return_dict=False), hidden_states, conditioning_hidden_states, num_frames=num_frames)[0]
                hidden_states = torch.utils.checkpoint.checkpoint(create_custom_forward(temp_attn, return_dict=False), hidden_states, num_frames=num_frames)[0]
                hidden_states = torch.utils.checkpoint.checkpoint(create_custom_forward(resnet), hidden_states, temb)
                hidden_states = torch.utils.checkpoint.checkpoint(create_custom_forward(temp_conv), hidden_states, num_frames=num_frames)
            else:
